%% previously s117  P89_XXXXX
%% now use s126 P57_XXXXX
cd T:\Imaging\Multimodal\MRF\Peter
load T1cm.mat
load T2cm.mat
s = 126;
cd Z:\Imaging\Multimodal\MRF\Recon_MRF_3T\Patients\(studyID)\Regis_files_wo_Q_bet
SYNf = load_untouch_nii('n_syN_T1w_(studyID)_Warped.nii');
syn = single(SYNf.img);
syn = syn(:,:,s)';
syn = flip(syn,1);
...
stp = 116;
% cd Z:\Imaging\Multimodal\MRF\Recon_MRF_3T\Patients\(studyID)\MRF_VBM
cd Z:\Imaging\Multimodal\Myelin\Patients\(studyID)
...
load T2cm.mat
s = 97;
cd Z:\Imaging\Multimodal\MRF\Recon_MRF_3T\Patients\(studyID)
